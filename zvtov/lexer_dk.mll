...
(*  $Id: lexer_dk.mll,v 1.4 2007-06-20 10:50:49 doligez Exp $  *)
{
open Parser_dk;;
open Lexing;;
...
let dkidbegin = [ 'A' - 'Z' 'a' - 'z' '_' ]
let dkidchar = [ 'A' - 'Z' 'a' - 'z' '0' - '9' '_' '\'' ]
...
rule dktoken = parse
  ...
  | dkidbegin dkidchar *  ('.' dkidbegin dkidchar*)*
  ...
  | "%%begin-auto-proof" inline*
      { BEGINPROOF }
  | "%%name:" space* (dkidchar+ as name) space*
      { BEGINNAME name }
  | "%%" dkidchar* ":" inline*
      { BEGINHEADER }
