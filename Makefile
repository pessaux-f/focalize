...
# DELIVERABLES=$(FOCALIZEC_NAME) $(FOCALIZE_DEP_NAME)
# DELIVERABLES_DIRS=$(FOCALIZEC_SRC_DIR) $(FOCALIZE_DEP_SRC_DIR)
#
...
.PHONY:\
 configure configure_external_tools configure_internal_tools\
 configure_deliverables
...
$(FOCALIZE_DEP_EXES): .done_build_focalizedep

.done_build_focalizedep: .done_build_focalizec
	@for i in $(ABSOLUTE_FOCALIZE_DEP_SRC_DIR); do\
	  echo "--> Building $$i ..." >&2 &&\
	  ($(CD) $$i &&\
	   $(MAKE) $(FOCALIZE_DEP_MAKE_ALL_TARGET));\
	  err=$$?;\
	  echo "<-- $$i [$$err]" >&2 &&\
	  case $$err in 0);; *) exit $$err;; esac;\
	done &&\
