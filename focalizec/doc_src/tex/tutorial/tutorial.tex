...
This document is a short tutorial for \foc, version 0.2.0 released in July 2009\footnote{\foc
can be downloaded from $\fid{http://focalize.inria.fr}$.}. It describes a full but simple
...
different sorts of entities. Delimitors (blank spaces, tabulations, line feeds) are often
required to separate syntactical constructs. Finally, the use in \foc sources of \ocaml or
\coq keywords (such as \code{Set}) or of standard library filenames (such as \code{list.v})
...
includes comments. However, for the sake of readibility, we will not use comments in our
...
Commands, filenames as well as outputs or error messages are in bold font, for example
...
The cost of developping formal specifications and proofs is compensated by the important gain
...
\foc, this is possible by using parameterization. Species can indeed be parameterized by an
...
before defining a species for subset, parameterised by an interface describing what is a
...
\end{lstlisting}
The first line of our code indicates that we use declarations and definitions from the file
...
notations (e.g. \code{#bool} or \code{bool} instead of \code{basics#bool}). Note that the
...
Filename & Produced by & Used by & Description\\\hline
...
follows -- it is provided after the keyword \code{proof}. In fact, we do not detail a real
...
that the validity of the latter depends upon the validity of the formers, and that there is
...
(proven using the definition), and to use these properties in proofs, instead of the
...
\textbf{subset.fcl}; the directives \code{use} or \code{open} can be used to access previous
...
The species \code{Subset} is parameterised by a collection \code{Val} which implements the
...
to overspecify the species and reduce the acceptable implementations (or logical models).
...
member of \code{s}. It could have been splitted in three different properties, but we here
...
tutorial such dependancies for now.
...
\code{ExtSubset} parameterised by a collection \code{Val}, which inherits of
...
is included in another, denoted \code{<:} in our code. Benefitting from the specification of
...
\end{lstlisting}
Any collection implementing \code{ExtSubset} will therefore have to provide a code for the
...
parameterise \code{Subset}, that is it is possible to consider subsets of a superset, but also
...
\end{lstlisting}
Defining \code{=} using \code{<:} is apparently relevant, and allow us to discharge the
...
lemmas to derive the complex results. Unfortunately, this complexifies the signature of the
species with numerous uninteresting results; in other word, the documentation of the species
...
goal is not derivable from the provided assumptions, and may be false.
...
We have used inheritance and parameterisation as composition operators between specifications,
...
and implement lists in an independant hierarchy of species, and then to use this hierarchy to
...
We edit a new file, named \textbf{mylist.fcl}\footnote{The filename \textbf{list.fcl} cannot
...
We start with a species \code{CoList} that represent a very abstract form of lists, possibly
infinite. As for \code{Subset}, \code{CoList} needs to be parameterised by a \code{Superset},
...
\end{lstlisting}
The \code{nil} and \code{cons} methods are used to build colists, while the \code{isnil},
...
very strong, indicating that the concrete implementation of any colist is either \code{nil} or
\code{cons}, a form of surjectivity of these methods w.r.t. the species.

Note that whereas subsets are things with a decidable membership, colists are things defining
a succession of values: membership is not specified\footnote{It may not be possible to browse
all elements in a colist to find a specific value.} but we have a function returning the head
element of a colist. Another characteristic of colists is that values may appear several time
...
Finiteness is a tricky concept to capture. Similarly to what is mentionned in Sub.
...
enforces any list to be buildable (accessible) starting from the list \code{nil} and using
...
improve the readibility of the proof. Let's start with a first subgoal:
\begin{lstlisting}
...
parameterised statement. Note that we do not claim that such a statement is valid: we just
introduce a name for a complex logical expression. The theorem \code{mem_del_mem}, on the
...
this result as a fact. The step \code{<1>f} concludes.

We can check that this proof structure is valid by invoking the compiler, before pursuing the
development. For the step \code{<1>1}, it is straigthforward:
\begin{lstlisting}
  proof = <1>1 assume v w : Val,
               prove mdm_(v, w, nil)
...
lists; yet rather than using the predefined \code{list} type provided in \textbf{basics.fcl},
...
\end{lstlisting}
Such an inductive definition claims that the type only contains values build from the
constructors (surjectivity), and that two values of this type are equal iff they are
structurally equal (injectivity); well-foundedness (finiteness of the constructs) is also
ensured.

\begin{tip}\small[Syntax] Type definitions are only authorised at top-level.
\end{tip}

This is also a polymorphic type: \code{'a} represents a type variable parameterising our
definition, \code{mylist(int)} or \code{mylist(char)} being examples of types obtained by
instantiating this variable. Note that the \foc syntax requires constructor identifiers to
...
parameterise \code{mylist} with \code{Val} (that is its representation).
...
the properties. Let's first address the higher-order \code{induction} property; as mentionned,
...
\code{Nil} and \code{Cons}. The \coq script is then expected to follow, between \code{\{*} and
...
\item inherits from \code{ExtSubset} and be parameterised by \code{IndList}.
\end{itemize}
...
In the second case, using parameterisation, we use lists to represent subsets, but emphasising
...
the implementation of subsets with the second approach, parameterisation by lists. As a
collection is never parameterised, and always implement one and only one complete species
whose parameters are intantiated by collections, we have to follow a strict discipline: to
...
represent executable implementations. They cannot be parameterised or inherited from, but they
...
As we want to create a collection from the parameterised species \code{ExtSubset}, we need
...
\end{lstlisting}
Remember that a collection cannot be parameterised, and that it has to implement a species
...
Having chosen to implement subsets as a species parameterised by lists, it is straightforward
...
as proven libraries for \ocaml programs. But it is also possible to use directly the various
...
species structure, a good recommandation is to multiply the inheritance steps and branches,
...
%----------------------------------------------------------------------------------------------
\subsection{Closure Reasonings}
\label{remark_closure}

It is worth mentionning that a species defines an interface which can be completed later,
...
implementation of our subset species has to be parameterised by a collection having an
...
returns an element of this list. Remember that beyond an equality, we have no operations on
...
\code{chc_congr}, but we need to slightly complexify our inheritance tree. For example, we can
...
\code{ExtSubsetChc} as well, but is parameterised by a collection providing a total order;
using this order, a normal form for subsets as well as a \code{chc} operator can be defined,
ensuring that the property \code{chc_congr} is indeed satified, for example returning the
...
As mentionned in Sub. \ref{specsubset_generic}, a subset is characterised by the values it
...
assumed or proven in \coq; yet this is just a technical concern. More fundamentally,
...
underlined, and collections are boxed. Note that parameterisation is not represented here to
...
Once finetuned, our development is more consistent and allows for easy extensions. This is
