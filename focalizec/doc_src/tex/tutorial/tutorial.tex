...
\date{July 2009 - Updated December 2012}
...
%----------------------------------------------------------------------------------------------
 ...
 This document is a short tutorial for \foc, version
 {\focalizeversion} released
in \month\ \year\footnote{\foc can be downloaded from $\fid{http://focalize.inria.fr}$.}.
 ...
 The \foc syntax is case sensitive, uppercase and lowercase identifiers being associated to
 different sorts of entities. Delimiters (blank spaces, tabulations, line feeds) are often
 required to separate syntactical constructs.
 ...
 using an emphasized font, for example \emph{collection}. Finally, mathematical statements are
 ...
 The cost of developing formal specifications and proofs is compensated by the important gain
 ...
 and a logical language. It is also object-flavored, developments being organized through a
 hierarchy of \emph{species}, build by \emph{inheritance} or \emph{parametrisation}. In
 ...
 properties and proofs, through a \emph{parametrisation mechanism}. The parameters are
...
given to the developer, that can choose between various approaches for example to favor
...
We therefore favor the generic approach, through a form of polymorphic specification. In
...
invoke species methods that we call ``species qua\-li\-fied notation''. In absence of
...
% NOT TRUE ANYMORE: SYNTAX WAS RELAXED.
...
transitivity, it is possible to prove the property
$\forall x, y,\ \ x = y \Rightarrow x = z \Rightarrow y = z$ about
\code{=} of the \code{Superset} species.

\medskip\noindent The sketch of the proof is informally the following.\\

\hspace{0.2cm} By hypotheses we have:

\hspace{0.2cm}\begin{tabular}{lr}
$x = y$ & (1) \\
$x = z$ & (2)
\end{tabular}

\hspace{0.2cm} From (1) by symmetry we prove that $y = x$ \ \ (3)

\hspace{0.2cm} From (3) and (2), by transitivity we prove that $y = z$ \ \ (Qed).

\medskip\noindent More formally, the underlying proof tree of this theorem is:
\begin{small}
\[\begin{array}{c}
\begin{array}{c}
\text{Trivial, by assumption}\\\hline
x = y,\: x = z \vdash x = y \\\hline
x = y,\: x = z \vdash y = x
\end{array}
\quad\quad
\begin{array}{c}
...
\\\hline
\begin{array}{c}
x = y,\: x = z \vdash y = x \wedge x = z
\\\hline
x = y,\: x = z \vdash y = z
\\\hline
\vdash
\forall\:x,\:y,\ \ x = y\Rightarrow x = z \Rightarrow y = z
\end{array}
\end{array}\]
\end{small}

In FoCaLiZe, thanks to \zenon, the definition and proof of this
property is simply achieved by:
\begin{lstlisting}
...
practically, membership defines a subset, as any subset is characterized by the values it
...
its behavior with regard to membership. As membership indeed characterizes a specific subset,
...
introduce very gradually various levels of specifications. Note that no\-thing enforces at this
...
Extensional subsets being subsets, we use the inheritance mechanism: we crea\-te a new species
...
equality; so we can emphasize this interpretation by adding the congruence property at the
...
A goal step is associated with assumptions (keyword \code{assume} to
introduce types or \code{hypothesis} to introduce logic formulae), a 
...
\code{qed by step <x+1>*}, i.e. all the steps of the level $x+1$,
whose aim is to prove the enclosing level $x$ goal).
...
indicates that to prove the equality \code{s = (s - v) + v} , we have to prove the
mutual inclusion. Invoking the
...
provide an implementation. Still favoring a very progressive approach in this tutorial, we
...
\end{lstlisting}
The definition for the method \code{mem} is recursive (keyword
\code{rec}), as computing \code{mem(v,l)} requires in some cases to compute
\code{mem(v, tail(l))}. In a formal development, one has to prove that
recursive functions always terminate to avoid logical
inconsistency\footnote{Consider for example the function
  \code{let rec f(n in Nat) = f(n)}: should it be a valid definition,
  it could have any return type -- and in particular it could be used
  as a   trick to build a value from an empty type.}.

Several termination proof kinds are being developed in FoCaLiZe (using a
measure, an order, a structurally decreasing argument). However the only
currently fully supported kind is stating that the recursion is structural,
i.e. applies on a strictly decreasing argument:

\code{let rec f(x, y, z)=... termination proof = structural y}.

The above code states that the function \code{f} is recursive
structural on its \code{y} argument. In other words, \code{y} is
stated (and must be) an argument starkly decreasing at each recursive
call.

Syntax exists for other kinds of proofs but is currently
ignored. For sake of simplicity, the termination proof is for now
admitted and is not required in the code (a termination proof is indeed
optional). The \foc compiler however warns that the species is potentially unsafe
% Didou
and will generate a pseudo-termination proof instead (i.e. an axiom),
in order to have the whole program however accepted by \coq. In some
sense, we afford lying to \coq, being optimistic about the part of the
consistency of our program involved by the termination of this
function.
...
\begin{tip}\small[Syntax] Type definitions are only authorized at top-level.
\end{tip}
...
\end{lstlisting}
In this case, we have explicitly provided a termination proof,
stating it as structural on the argument \code{l} (i.e.  \code{l} is
the strictly decreasing argument).
...
In the second case, using parametrisation, we use lists to represent subsets, but emphasizing
...
the implementation of subsets with the second approach, pa\-ra\-me\-tri\-sa\-tion by lists. As a
...
therefore create a new species with me\-thods to import or export values:
\begin{lstlisting}
...
\end{lstlisting}
\code{Int_Coll} now denotes a form of abstract data type, whose representation is hidden. It is not
possible to inherit from \code{Int_Coll}, or to modify otherwise its
structure; of course, it is still possible to use \code{Int_Coll} as a parameter.

\begin{tip}[Implementation] Collections are the final entities of \foc developments and
represent executable implementations. They cannot be
pa\-ra\-me\-tri\-sed or inherited from, but they can be used as
...
provide more features -- recognizing that it may causes unnecessary burden for the developer
...
operation. Yet the complement ope\-ra\-tion would also be part of all the other inheriting
...
species \code{Subset} only describes finite subsets -- nothing
prevents the developer from introducing
through inheritance more powerful methods such as a complement operation. The fact
...
$\fid{chc}(S_1)\!=\!\fid{chc}(S_2)$. Of course, we can prevent those strange behaviors by
...
equality \code{basics#( = )}, there is no way to characterize a good one and a bad one. If we
...
emphasize the freedom offered to a developer facing an abstract specification, provided this
specification is appropriate.

As mentioned in Sub. \ref{specsubset_generic}, a subset is characterized by the values it
contains, in other words a subset is characterized by its membership function. It is therefore
