% $Id: refman.tex,v 1.8 2008-10-17 12:07:24 pessaux Exp $
...
\begin{itemize}
  \item The {\bf carrier}\index{carrier}. This is a type definition
    that defines the type of the values embedded in the species. When
    ``instantiated'', a species will lead to {\bf a value having this
    type}. This especially means that an instantiation of a species is
    NOT, like in Object-Oriented languages an entity embedding data and
    code. The instantiation of a species will be a
    {\bf value}. The carrier is named {\tt rep}\index{rep}. For
    instance, we can define a species that implements couples of
    native integers data structure as:
    {\scriptsize
      \begin{lstlisting}
species IntCouple =
  rep = (int * int) ;
...
parametrise our species by the {\bf value 8}, or at least by a
...
If a species {\tt S1} inherits from a parameterised species {\tt S0},
...
parameterised, it can use its own parameters to do that. Assuming we
have a species {\tt List} parametrised by a collection parameter
representing the kind of elements of the list. We now want to derivate
...
This mechanism enable two programming features:
\begin{itemize}
