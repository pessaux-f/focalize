% $Id: constructs_syntax.tex,v 1.25 2008-12-07 23:49:49 weis Exp $
...
to a type during its definition. \focal\ provides the following basic builtin
types (constructors):
\begin{itemize}
...
capitalised name since species names are so. The special case of
...
As stated before, each type {\bf definition} leads to a new type
{\bf constructor} that belongs to the set of type constructors that
...
They provide the way to create new {\bf values} that belong to the same {\bf
  type}. Like {\tt 1} or {\tt 42} are {\bf values} of {\bf type} {\tt int},
one may want to have {\tt Red}, {\tt Blue} and {\tt Green} as the {\bf only}
values of a {\tt color} type. We say {\bf only}, since it means that the
new {\tt color} type will be inhabited only by these 3 (wished)
values. To defined such a type, we itemize its value names (that must be
capitalised identifiers), each one preceded by a ``$\mid$'' character :
{\scriptsize
\begin{lstlisting}
type color =
  | Red
...
Values of a sum type definition are built from its {\bf value constructors},
i.e. from the names enumerated in the definition of the type. A {\bf value
  constructor} must not be confused with a {\bf type constructor}: as
expected a value constructor builds a {\bf value}, while a {\bf type}
constructor builds a type. For instance, {\tt color} is a type
constructor and {\tt Red} is one of the {\bf value} constructors of the type
{\tt color}.

It is possible to define {\bf parameterised} sum type constructors. In this
case, the constructors may be functional and have a {\bf value} argument. For
instance, let's define the type of playing cards as king, queen, jack and
simply numbered cards:
{\scriptsize
\begin{lstlisting}
type card =
  | King
...
It is possible to parameterise constructors by any type expression,
even recursive.
...
{\tt Cons ((false, 2), (Cons ((true, 1), Empty)))}. The length of this list is
...
section. Assuming a variable {\tt v} was bound to the resulting
...
of evaluation of its body expression) and its environment (i.e. the set of bound
variables that appears in the body of the function). This closure is a
constant, that is never modified and not even used until the function is
applied, as described below in \ref{function-application}.



\medskip
Identifiers names depending on the kind of entity they are bound
...
parameters in a function body, or bound parameters in {\tt let-in} and {\tt
  match-with} constructs). If no variable definition with this name is found,
...
definitions imported by the {\tt open} directives are examined to find
the name. Finally, if no definition is found, the
identifier is reported as unbound by an error message. Note that
...
``toplevel'' definition exists for an entity {\tt foo}, and if afterwards an
{\tt open} directive imports another definition for {\tt foo}, then the last one
is retained.



\medskip
In case of ambiguity, an identifier can be annotated with its defining
compilation unit using the sharp (\#) notation (see \ref{qualified-name}).
...
\medskip
In addition to this multi-branches conditional feature, the
...
will load the definitions of the file ``sets.fcl'' in the current name
...
\index{theorem}
\index{property}
Like introduced in \ref{species-intro}, properties are first order logic
...
species S =
  ...
  theorem symmetry : all x y in Self, Self!equal (x, y) -> equal (y, x)
...
\end{lstlisting}
    ...
    absence. Although such proofs are not satisfactory in term of
    ...
    \begin{itemize}
      ...
      \item Second case deals with import of external code, i.e. code
        ...
        \focal\ universe can not be modelled by \focal. It only can
  ...
  \item [{\bf Write an automated proof script}] \focal\ provides a
    ...
    goals, then to prove these hypotheses, hence building a proof tree
