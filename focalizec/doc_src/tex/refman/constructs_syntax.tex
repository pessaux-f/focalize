% $Id: constructs_syntax.tex,v 1.29 2008-12-16 11:19:02 pessaux Exp $
...
expressions, i.e. a kind of function returning a logical proposition.

For instance we may want to express that for a certain $x$, and a
certain $y$, $x$ is lower than $y$ and that $x+y<10$.

A first approach would be to think to a logical proposition like:
$\forall x, y : {\tt int}, x < y /\setminus x+y<10$. But this is wrong
since this does not hold for any $x$ and any $y$.

Trying $\exists x, y : {\tt int}, x < y /\setminus x+y<10$ is not
anymore a solution since this doesn't express the fact that we want
one day to chose a particular $x$ and a particular $y$.
...
\focal\ provide the {\tt logical let} construct allowing to achieve
such definitions. Our example would be expressed by:
{\scriptsize
\begin{lstlisting}
use "basics" ;;
open "basics" ;;
...
Since {\tt logical let} binds an identifier to logical expression, the
body of the definition {\bf must} obviously {\bf be of type}
{\tt bool}. Once defined, {\tt f} can be used as a regular function in
