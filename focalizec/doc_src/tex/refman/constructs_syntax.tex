...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Mutually Recursive Types Definitions}
\TODO{Only sums. Syntax: \lstinline{type} \ ldots \lstinline{with} \ldots}
...
\subsubsection{{\tt final (logical) let}}
\TODO{Cannot be redefined by inheritance.}
...
\item {\bf Write an automated proof script}.  {\focal} provides a
  ...
  may be proved by the theorem prover. Without entering deeply
  ...
  methods. Then the prover tries to automatically guess a proof of this
...
\end{lstlisting}}
  ...
  \item {\bf Write a {\coq} or {\dedukti} script} This way is the most difficult
     since it means to directly write {\coq} or {\dedukti} code. It
    requires the understanding of both the proof checker and the mapping the
    {\focal} compiler does to generate {\coq}/{\dedukti} code from {\focal} source
    code. The section \ref{focal-coq-mapping} describes % [EJ] Reference mal formee
     how {\focal} definitions are mapped onto
    {\coq} and {\dedukti} names.

The script is introduced by the keywords {\tt coq proof} or {\tt dedukti proof} and
