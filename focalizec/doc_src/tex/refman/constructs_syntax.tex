...
exp & ::= & integer-litteral\ \mid\ string-literal & \\
    &     & \mid\ character-literal \mid\ float-literal & \\
    &     & \mid\ {\tt true}\ \mid\ {\tt false} & Booleans \\
    &     & \mid\ [[ident] \terminal{#}]uident &
                       File-qualified sum type constructor \\
    &     & \mid\ [\terminal{Self}] \terminal{!} uident &
                       Method of the current species \\
    &     & \mid\ [[lident] \terminal{#}] [uident \terminal{!}] lident & \\
    &     & \mid\ [[lident] \terminal{#}] [uident \terminal{!}]
               \terminal{(} operator \terminal{)} & Infix or prefix
            operator used in functional position \\
    &     & \terminal{let}\ [\terminal{rec}]\ let-decl \terminal{=}
               exp\ \terminal{in}\ exp & \\
\end{syntax}
\vspace{0.2cm}
