% $Id: constructs_syntax.tex,v 1.42 2009-03-25 15:57:59 pessaux Exp $
...
{\bf type definitions}.
Types constructors can be parametrised by  {\bf type expressions}
...
types can be {\bf parametrised} by a type expressions, corresponding
...
\end{lstlisting}
  ...
  You may notice the 2 sets of parentheses. In effect, the basic syntax
  for constructor arguments implies surrounding the arguments be
  parentheses. The type expression denoting a tuple surrounds the
  components of the tuple by parentheses. Hence combining these two
  things, we now understand why a tuple value parametrising a
  constructor must have 2 layers of parentheses.

\item Or the constructor has {\bf several} arguments. In this case, since
  as state above the basic syntax for constructor arguments implies
  surrounding the arguments be parentheses, a type definition like:
{\scriptsize
\begin{lstlisting}
type t2 =
...
let fct_t2 (x) in mytuple =
  match x with
   | Cstr (x) -> x
;;
(* Is accepted since the type mytuple aliases a 3 components tuples
and Cstr is really parametrised by 1 argument that is a 3 components
tuple.
...
means that replacing variables in $t_1$ and replacing variables in
$t_2$ leads to a same type.
...
type
$t'_2 = {\tt bool\ } \rightarrow {\tt\ int\ } \rightarrow 'b \rightarrow 'c$.
...
type variables. Only part (or none) of them may be sufficient.
...
% addind spaces is not a good practice: it is mandatory! In focalize,
...
in \ref{properties-theorems-proofs} are first order logic
...
Suppose we want,  for a certain value of $x$ and $y$, to use the fact
