% $Id: constructs_syntax.tex,v 1.27 2008-12-11 15:56:36 hardin Exp $
...
collections. Hence a type definition is visible in the
...
to a type during its definition. \focal\ provides the basic built-in
types (constructors):
\begin{itemize}
...
A type expression can also denote the representation of a species or a
collection  by using
their name, thus a 
capitalized name. The special case of
{\tt Self} represents the representation of the current species. Hence,
...
As stated before, each type {\bf definition} introduces  a new type
{\bf constructor}  (the name of the type)  thus available to build 
new type {\bf expressions}. Hence, defining a type is the
...
It is possible to define {\bf parametrised} value constructors of sum types. In
...
written on the card. Some values of type {\tt card} are: {\tt King}, {\tt
  (Numbered 4)}, {\tt (Numbered 42)}.

It is possible to parametrise sum type value constructors by any type
expression, even recursive.
...
of this constructor). This is a  value of type {\tt b\_i\_list}:
...
can be instantiated by any type expression. For instance, the type
...
(quote) character. In the above definition, we see that the value
...
Aliases provide a way to create type abbreviations. It it common to
...
As previously introduced (c.f. \ref{rep-is-method}) the representation
is a special kind of method of a species that describes the internal
data structure that the species
manages. Hence, it is a kind of {\bf type definition}, more accurately
an {\bf alias type definition}. This means that a representation {\bf
  does not intrduce a new type}, it only ``assign'' to the representation a
{\bf type expression} defining the type of the manipulated entities of
the species. Moreover, like for any other methods
(c.f. \ref{no-polymorphism-for-methods}), the representation must not be
polymorphic. This means that its definition  cannot contain type
variables (but may contain collection parameer names). Defining
a species'representation is simply done by adding the {\tt representation} method:
{\scriptsize
\begin{lstlisting}
open "basics" ;;
...
of \focal\ dealing with them in this section. They will be considered 
in a next section, in \ref{making-proofs}. Indeed proofs are
...
An identifier expression (also called a ``variable'') is evaluated into
the value bound to this variable. A variable is said {\bf bound} by
its definition, i.e. the construct that assigned it a value. In
\focal\, there are three ways to bind a variable:
\begin{itemize}
...
section. Assuming a variable {\tt v} is bound to the result

{\bf value} of the computation of the {\bf expression} {\tt 2 + 5},
then each occurrence of {\tt v} in a expression will be ``replaced''
by 7 in this expression. This is basically the principle of the so-called
{\bf eager} or {\bf call by-value}
evaluation regime.
\label{functional-value}
\index{functional value}
Like in any functional language, a function definition binds an
identifier to a functional value that is the evaluation of the
body of the function. Such a value is slightly different from
...
variables present in the scope of the function). This closure will be
...
Identifiers forms differ depending on the syntactic class of entity they refer
to. In the simplest form, an identifier expression is a lowercase
identifier that refers to a value. The \focal\ compiler will determine its
definition from the current scoping context.
...
parameters  in a function and {\tt let-in} and {\tt match-with}
...
method is found with this name, it will be retained, otherwise the identifier
is looked in the preceding ``toplevel'' definitions of the current
compilation unit.
...
identifier is reported unbound by an error message. Note that an
{\tt open} directive may arise anywhere at topleve' in the source
...
{\tt meth} of the current species. It is possible to explicit
{\tt Self} in the naming scheme using {\tt Self!meth}. This
is useful when a more recently defined identifier hides
a method of the species at hand:
{\scriptsize
\begin{lstlisting}
species S =
  let m (x in ...) = ... ;
...
binds {tt x} to the evaluation of the {\bf expression} {\tt 5} (i.e. the
integer {\bf value} $5$) and then, the
evaluation of the body returns the tuple {\bf value} $(5, 5)$.
From the syntax, it is clear that {\tt let-in} constructs can be nested.
For instance,
{\scriptsize
\begin{lstlisting}
let x = 5 in
...
The notion of ``binding of an ident to a value'' is essentially different from the
...
once and for all: it is impossible to change its ``assigned'' value, once it
has been bound. In other words, we have no assignment construct.
Each new definition, binding an already bound identifier will just hide the
old definition from the point of the new definition. For instance:
{\scriptsize
\begin{lstlisting}
let x = 5 in
  let y = (x, x) in
...
clearly appears that the first value bound to {\tt x} holds until it
...
\medskip
The {\tt let-in} construct serves to bind an identifier to a value.
As a consequence, it can also bind an identifier to a functional value.
This lead to the natural way to define {\bf functions}.
As expected, the bound identifier is no more a simple constant value: it now
has arguments. For instance:
{\scriptsize
\begin{lstlisting}
let f (x, y) = x + y in
f (6, 7)
...
The {\bf let} construct binds {\tt f} to a function with 2 parameters
{\tt x} and {\tt y}, and the body of {\tt f} is the addition of these 2
parameters. Then the body of the {\tt let-in} construct
applies {\tt f} to 2 effectives arguments {\tt 6} and
{\tt 7} (we obviously expect the result of this {\em application} to be $13$).
(Function application is explained below in \ref{application-expression}).

It is possible to provide a type constraint to precise the type of the return
value of a function, or the type of the {\tt let}-bound variable or parameters:
...
% FIXME
% Hey, this is very strange! The let ... and is no more commutative !!!
% I don't agree! And gives no notion of sequentiality as let in does...
...
expression. When {\tt a + 5} {\bf is equal} to $0$ the result of the
...
identifier  {\tt g} to a {\bf functional value}. Then the application
expression {\tt g (true, 1)} is evaluated. So the values of {\tt g}
and of the expression {\tt (true,1)} are elaborated: 
{\tt true} is evaluated into the boolean {\bf value} $true$, {\tt 1}
into the integer {\bf value} $1$. The next step is 
to evaluate the body of the {\bf functional value} of {\tt g},
...
records, leaving the other fields unchanged. If the record type
...
expressions are used. It is still possible to have top-level
expressions that will get directly evaluated and not bound to any
...
introduced by the keywords {\tt property} and {\tt theorem} and may
 contain expressions. A dedicated section in
...
\medskip We also made reference to toplevel-definitions,
i.e. definitions outside any species. General functions and general
theorems, i.e. that do not depend on a particular species can be
introduced as toplvel-definitions. Toplevel-functions are introduced
by the {\tt let} keyword and don't have a ``{\tt in}'' expression,
this part being implicitly the remaining of the program (i.e. the
current compilation unit and those using the current).  Toplevel-theorems are
introduced by the {\tt theorem} keyword. As already stated in
\ref{topleve-def}, these definitions must be ended by a double semi
(``{\tt ;;}'').  {\scriptsize
\begin{lstlisting}
let is_failed (x) =
  match x with
...
This directive load the definitions of the file ``sets.fcl'' in the
current name resolution (scoping) environment.
...
As instroduced in \ref{species-intro}, properties are first order logic
propositions and theorems are properties with their proofs. We will
study here first the structure of logical expressions used to express
the statements, show properties and theorems forms and shorty present
...
using this keyword may help.
  \begin{itemize}
      ...
      \item Last case addresses ``well-known'' mathematical properties
        ...
        example is the fact that since an integer is coded on a machine
...
\item [{\bf Write an automated proof script}] \focal\ provides a
  ...
  into the syntax further described in \ref{making-proofs}, the main
  features are the following. The user may state hypotheses,
  ...
  goal, then tries to prove those lemmas, hence building a proof
  tree until the top goal (i.e. the theorem) is proved. Below follows
  an example of such proof. 
...
toplevel-theorems. 
Unlike theorems, properties can't appear at
