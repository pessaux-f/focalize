...
pattern & ::= & integer\_literal & \\
    ...
    &     & \mid \terminal{(} pattern\ \{ \terminal{,}\ pattern\}+
                  \terminal{)} & Tuple \\
    &     & \mid \terminal{(} pattern \terminal{)} & Parenthesed pattern
\end{syntax}
...
constructor. The obtained value, after evaluation, is writen by the
name of the constructor followed by the tuple of values provided as
arguments. For instance, with the folowing type definition:
{\scriptsize
\begin{lstlisting}
type t =
  | A
...
\begin{itemize}
  \item By a {\tt let\ ldots in} construct,
  \item By a toplevel definition ({\tt let} or {\tt theorem}),
  \item by a method definition,
  \item by a pattern inside a {\tt match \ldots with} construct,
...
\medskip
Identifiers forms differ depending on the classe of entity they refers
...
ones imported by the {\tt open} directives are examinated to find one
...
where unfortunately {\tt (*} is parsed as a begining of comment,
leading to a syntax error. The symetry obviously arises with the
