% $Id: constructs_syntax.tex,v 1.45 2009-06-04 15:11:07 jaeger Exp $
...
\alt \tok{Self}
\alt \tok{'}\nt{lident}
\alt \nt{uident} \tok{(} \repsep{\nt{type-exp}}{\tok{,}} \tok{)} % [EJ] TRES TRES LOUCHE A QUOI CA SERT?
\alt \nt{type-exp} \tok{->} \nt{type-exp}
\alt \tok{(} \repsep{\nt{type-exp}}{\tok{*}} \tok{)}
\alt \tok{(} \nt{type-exp} \tok{)}
\end{syn}
...
``several'' arguments. \focalize % [EJ] Probleme de separateur, utiliser \xspace!
...
\begin{itemize}
  ...
  \item by a method definition  ({\tt let}), % [EJ] Pourquoi au top-level un theorem binde-t-il, mais pas dans une espece? Est-ce une forme de logical let?
 \end{itemize}
...
% [EJ] L'exemple me semble faux, a corriger
...
\ref{application-expression}). % [EJ] Reference incorrecte
...
;;
\end{lstlisting}
} % [EJ] Ne marche pas, bien sur, mais je ne sais pas comment corriger
...
% [EJ] Il faudrait un peu de detail sur la recherche dans le PATH
% Supposons que je fasse un use "u", focalizec cherche le fichier
% u.fo, mais quid si il y en a plusieurs?
...
\begin{syn}
\nt{logical-exp} \is
     \tok{all} \repsep{\nt{lident}}{\tok{,}} \tok{in} \nt{type-exp} % [EJ] Semble louche, comment doit-on parser {lident}(,)+ ? On ne repete que la , ? La , doit toujours apparaitre meme apres le dernier lident ? Je signale aussi qu'on ecrit all x y in Self et pas all x,y in Self ! Je suggere donc plutot {lident}+
     \tok{,} \nt{logical-exp}
\alt \tok{ex} \repsep{\nt{lident}}{\tok{,}} \tok{in} \nt{type-exp}
     \tok{,} \nt{logical-exp}
\alt \nt{logical-exp} \tok{->} \nt{logical-exp}
