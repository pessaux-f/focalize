...
%    used  to define others {\em methods} while
  ...
  \item Theorems ({\tt theorem}) made of a  name, a statement and a
    ...
    accompanying the statement will be processed by {\focal}, a
    first-order theorem prover (either {\zenon} or {\zmod}) and
    ultimately checked with a proof checker (either the {\coq} proof
    assistant or the {\dedukti} logical framework).
...
done by the developer; the compiler ultimately send them to the proof checker
for verification: all the demonstrations made in {\focal} are automatically
machine checked for consistency (by {\coq} or {\dedukti}).
...
generate a complete proof and exhibit a {\coq} or {\dedukti} proof term suitable for
...
or {\dedukti} proofs. Comfort of automation is lost in favor of an increase in expressive power:
the entire {\dedukti} and {\coq} vernacular languages are now available to the developer to
write the proof.
...
{\focal} currently compiles programs toward three languages, {\ocaml} to
get an executable standalone, and {\coq} and {\dedukti} \cite{focalide,} to have a formal model
...
In contrast, in {\coq} and {\dedukti}, all the {\em methods} are compiled,
i.e. ``computational'' {\em methods} and logical {\em methods} with their
proofs. This allows the proof checker to check the entire consistence of the system
developed in {\focal}.
