% $Id: lexical_conventions.tex,v 1.7 2009-01-21 17:49:32 weis Exp $
...
In particular, this means that a {\bf source file cannot end by an
annotation}.
...
\begin{itemize}
  ...
  \item {\bf Infix/prefix operators}.
    ...
    is free to define them and use them as ordinary
    prefix/infix operators instead of only using prefix function names and regular function
    application. For instance, if you do not like the {\focal} predefined {\tt ^}
    operator to catenate strings, you can define a new infix synonym for {\tt ^},
    say {tt ++}, using
{\scriptsize
\begin{lstlisting}
let ( ++ ) (s1, s2) = s1 ^ s2 ;
...
\end{lstlisting}
    ...
    The notation {\tt ( op )} is named the {\em prefix form notation} for {\tt op}.

    Since you can only define prefix identifiers in {\focal}, you must use the
    prefix form notation to define an infix operator.
    ...
    to apply it in prefix position as a simple regular function (with a
...
\end{lstlisting}
    ...
    redefine the {\tt *} operator without spaces around the operator: you
...
\end{lstlisting}
    ...
    {\bf Attention} : since the comma character is a possible
    starting character for an infix, when using commas inside tuples
    be sure to put a blank behind the comma. Otherwise, confusion arises
    {\tt (0, 1,n)} is understood as a couple of two values:
    integer 0 as first component, and a second
    component taht is the infix application of operator {\tt ,n} to
    the integer 1 and \ldots nothing, hence leading to a syntax error.
