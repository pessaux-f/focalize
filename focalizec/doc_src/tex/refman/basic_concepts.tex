% $Id: basic_concepts.tex,v 1.8 2008-12-05 12:54:00 pessaux Exp $
...
\label{topleve-def}
\index{toplevel} 
In the remaining of this manual, we will call {\bf toplevel
definition} a definition appearing outside species and
collections. Such definitions car only be:
\begin{itemize}
  \item Species,
  \item collections,
  ...
  \item expressions to directly evaluate (hence not really definitions
  since their value is not bound to any identifier).
\end{itemize}

As any \focal\ ``toplevel'' definition, a species is
...
\begin{itemize}
  ...
  \item The {\bf carrier}\index{carrier}. This is a type definition
    ...
    named {\tt rep}\index{representation}. For instance, we can define a species
