      ...
      (fun  ((_, param_ty_coll), (param_name, param_kind)) ->
        match param_kind with
         | Types.CCMI_is ->
             ...
             (Env.ScopeInformation.SPK_is, Parsetree.Vuident param_ty_coll, None)
         | Types.CCMI_in _ -> (
             (* We generate the parameter and its type. *)
             (Env.ScopeInformation.SPK_in, Parsetree.Vlident param_name, Some param_ty_coll)
            ))
...
let dump_collection_generator_arguments_for_params_carriers out_fmter lst ~param =
  Format.fprintf out_fmter "(; %d params, %swith types ;)"
                 (List.length lst)
                 (if param then "" else "not ");
  List.iter
    (fun (param_kind, param_name, ty_opt) ->
      match param_kind, ty_opt with
       | Env.ScopeInformation.SPK_is, _ ->
          if param then
            ...
            Format.fprintf out_fmter "@ _p_%a_T"
              Parsetree_utils.pp_vname_with_operators_expanded
              param_name
       | Env.ScopeInformation.SPK_in, Some ty when param ->
           (* One must use the parameter of the collection generator to
              ...
              separated by "_". *)
          if param then
           Format.fprintf out_fmter "@ (_p_%a_%a : cc.eT _p_%s_T)"
             Parsetree_utils.pp_vname_with_operators_expanded param_name
             Parsetree_utils.pp_vname_with_operators_expanded param_name
             ty
       | Env.ScopeInformation.SPK_in, _ ->
...
let generate_collection_generator ctx print_ctx env compiled_species_fields
  ...
  (* Now, we dump the parameters of the collection generator. *)
    ...
    if species_descr.Env.TypeInformation.spe_is_closed then
      ...
      let coll_gen_params_info = {
        ...
        (* Just remove the "IN"/"IS" tag that is not needed to keep in the code
          ...
          List.map (fun (_, x, _) -> x) params_carriers_abstr_for_record;
