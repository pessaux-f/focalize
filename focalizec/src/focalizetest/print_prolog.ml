...
let pretty_print_list_comma output_formatter l f =
  pretty_print_list output_formatter
    l f (fun _e -> fprintf output_formatter ",@ ")
;;


let pretty_print_list_sep_comma output_formatter l f f_sep =
  pretty_print_list_sep output_formatter
    l f f_sep (fun _e -> fprintf output_formatter ",@ ")
;;
...
(** A prolog term is an infix operator if and only if it has arity 2 and is in
    the [priority_list].
    [is_infix f_name args] returns [true] if [f_name] applied to the arguments
    [args] is an infix operator. It returns [false] otherwise. *)
let is_infix f = List.mem_assoc f priority_list ;;


(** A we take the priority of an operator from its name. If the function name
passed in argument is not an operator returns [-1]. *)
let prior_of f = try List.assoc f priority_list with Not_found -> -1 ;;
