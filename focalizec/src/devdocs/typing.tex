% $Id: typing.tex,v 1.16 2012-11-13 13:17:15 pessaux Exp $
...
disappears from the species structure. Obviously, we still have means
...
\section{Environment and structures for the typing pass}
...
\index{species!parameter!typing}
...
\subsubsection{Not forgetting constraints from inheritance}
It is roughly described in Virgile's work that during fusion, methods
inferred types and possibly inherited ones are matched against
together to determine the final type of methods. This is not
sufficient. In effect, let's consider the following code:
...
While in {\tt S2}, typing {\tt join}, we already typed {\tt to\_int}
but with type ${\tt int} \rightarrow {\tt int}$ ! In effect, the
fusion pass is not yet done for species {\tt S2} since it will be done
once the species has all its methods typed. Hence, when typing the
method {\tt join}, we see it calls {\tt to\_int}, so {\tt join} also
has type ${\tt int} \rightarrow {\tt int}$.

Then comes the fusion that really gives {\tt to\_int} the good type:
${\tt Self} \rightarrow {\tt int}$. But, it's too late for {\tt join}
which is not impacted by the fusion (no signature for it defined above
in the inheritance), and remains with its previous type.

It is clear that's not a question of missing dependency on {\tt Self} in the
dependencies calculus causing wrong abstractions
($\lambda$-liftings). In effect, the type found for {\tt join} is
${\tt int} \rightarrow {\tt int}$ which is totally wrong since {\tt to\_int}
has type ${\tt Self} \rightarrow {\tt int}$, so the argument {\tt x}
of {\tt join}, passed to {\tt to\_int} {\bf must} be {\tt Self} an not {\tt int} ! 

The solution is to notice that while inferring the type of {\tt to\_int},
we didn't "compared" the found type to the already existing
type due to signature brought by inheritance. So, when typing a
method, trying to match it agains a possibly already existing type constraint
can solve the issue. That's what is done: if a scheme is found in the
environment (the initial environment, not the one with pre-bindings),
then we take an instance of this scheme and unify the inferred type
with it, retaining the result of the unification as final type for
the method. If none is found, we simply keep the inferred type.
...
\medskip
{\bf A question on the fly}: While writing the document, I wonder if
this collapsing process wouldn't be carried out in the fusion
algorithm used to create the normal form, exactly like when we pair
signatures and {\tt let}-definitions\ldots
...
modulo $\alpha$-conversion of the bound identifiers. In particular, we
...
During this fusion operation, when we ``fusion'' a signature and a
{\tt let}-definition, if the unification succeeds, then we
{\bf keep the type given by the signature} to assign it to the
method. We don't keep the type infered for the method. In effect, the
inference algorithm may find a type that is more general than the
signature, or may leave some type occurrences expanded althgough they
are in fact occurrences of {\tt Self} (obviously because there was
previously de definition of {\tt representation} equal to type. For
instance, if we have:
{\footnotesize
\begin{lstlisting}
species A0 =
...
\noindent, the inference algorithm will find in {\tt A1} that {\tt f}
has type $\forall \alpha.\alpha \rightarrow \alpha$ since this is the
most general unifier. And because polymorphic methods are not allowed,
if we do not perform signature matching for {\tt f}, then first, we
won't see that it must be considered to be
${\tt Self} \rightarrow {\tt Self}$, then second we will reject the
method. Hence this point shows that verification that methods are not
polymorphic must be done {\bf after} the definitions are assigned the
type of their possible signatures.
...
Since nowhere in the definition of {\tt f} in {\tt B1} {\tt Self}
appears, the type inference algorithm will see that {\tt 1} has type
{\tt int}, that {\tt +} as type
${\tt int} \rightarrow {\tt int} \rightarrow {\tt int}$ and then will
deduce that {\tt f} has type ${\tt int} \rightarrow {\tt int}$.
Unfortunately, the developer wanted to see {\tt f} has a function
injecting an integer into the species data-type, hence wanted to
consider it with type ${\tt int} \rightarrow {\tt Self}$. So if we
don't keep the type of the signature to assign it to the definition,
we forget this constraint information.
...
\index{type!definition!external}
