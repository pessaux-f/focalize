% $Id: codegen_model.tex,v 1.28 2012-11-13 13:17:15 pessaux Exp $
\label{code-gen-model}
...
\item Finally come all the entity parameters then the methods of the
  species parameter appearing
...
(however, their type can appear -- imagine simply a method returning the
entity parameter).
...
\item The \index{$\lambda$-lift!methods of {\tt Self}}
  ...
  method that depends on. Otherwise, there would be no link between
  the fact the method depends on a definition, and a $\lambda$-lifting
  would represent any definition that will be provided one day,
  nobody know when and where ! This deals with the first sentence of
  the last paragraph of page 116 (below definition 58) in Virgile
  Prevosto's PhD.
\end{enumerate}
...
\noindent Once compiled to \ocaml\, we get the following code:
...
If we check the interface of the \ocaml\ compilation unit, we can see
...
\index{external!type definition}
...
\index{external!value definition}
