...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Le type des champs du record}
...
ce paramètre (ex. {\tt 'a0\_as\_carrier} pour le paramètre {\tt A} de
...
\begin{itemize}
  ...
  \item print
\end{itemize}
On retrouve alors directement des fonction OCaml de même nom dans le
module. Ces fonctions sont agrémentées de paramètres supplémentaires
par rapport à leur homologues FoCaL. Pour chaque méthode dont une
...
appliquant les générateurs de méthodes de chaque méthode aux fonctions
...
endroit. Ceci sera mémorisé dans une structure persistante de la passe
...
supplémentaires dus aux méthodes de nous-mêmes dont on pourrait
dépendre !
...
dans l'espèce, on reprend directement sont générateur de méthode qui
...
courante (pour les méthodes de nous-mêmes dont on dépend). Donc on
...
généré et dont nous avons parlé au début et on leur affecte le nom de
...
\begin{itemize}
  \item {\tt env} : L'environnement courant contenant les informations
    persistantes de compilation. C'est un {\tt Env.MlGenEnv.t}. Dedans
  ...
  \item {\tt species\_descr} : La structure représentant l'espèce
...
\begin{enumerate}
  ...
  \item Génération du type record représentant l'espèce, pas son {\em
  ...
  \item Si l'espèce est totalement définie, on génère un générateur de
    collection (fonction {\tt generate\_collection\_generator}) et on
    récupère l'information disant quels arguments lui passer afin de
    pouvoir appeler ce générateur de collection ultérieurement.
...
pour OCaml qui considérerait que ce sont des constructeurs de types
...
\begin{enumerate}
  ...
  \item Lorsque l'on travaille au sein d'une collection (pas une
    ...
    espèce qui n'est pas une collection, lorsque l'on parle du type
    ...
    Toutes les méthodes faisant apparaître {\tt 'me\_as\_carrier} vont
    se retrouver, dans le générateur de collection instanciées par des
    fonctions ayant pour type {\tt me\_as\_carrier}. Et donc
    ...
    qui se trouvait dans une espèce va se retrouver dans une
  ...
  \item Finalement, on clôt la définition du type record.
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Cette fonction génère le code OCaml pour un champ de méthode
(c'est-à-dire pour une construction {\tt let} ou un paquet d'items de
{\tt let rec}). Elle ignore les {\tt sig}, les {\tt theorem} et les
{\tt property} qui eux ne donnent naissance à aucun code OCaml. Cette
fonction est itérée à la demande sur tous les champs de l'espèce.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Algo}
\begin{enumerate}
  \item Si le champ est une {\tt sig}, un {\tt theorem} ou une
    {\tt property}, on ne génère pas de code.
  \item Si le champ est un {\tt let} non récursif on détermine les
    paramètres à lambda-lifter (fonction
    {\tt compute\_lambda\_liftings\_for\_field}) et on entre dans la
    génération du code du binding (fonction
    {\tt generate\_one\_field\_binding}). \\
    À l'issue de la production de code, on enregistre les informations
    de paramètres lambda-liftés dans une structure qui sera persistante
    à la passe de génération de code OCaml.
  \item Si le champ est un {\tt let rec}, alors on commence par
    récupérer pour chaque fonction récursivement liée, les
    informations sur ses paramètres lambda-liftés et on lie ces
    information à chaque fonction dans l'environnement afin, en cas
    d'appel récursif, d'être capable de savoir quels sont les
    paramètres à rajouter à l'appel. \\
    Ensuite, on produit successivement le code de chaque
    identificateur lié par le {\tt let rec}. \\
    Et comme ci-dessus, à l'issue de la production de code, on
    enregistre les informations de paramètres lambda-liftés pour chaque
    identificateur lié par le {\tt let rec}.
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{compute\_lambda\_liftings\_for\_field}
Cette fonction préprocesse un champ avant sa compilation vers
OCaml. Elle calcule différentes informations relatives aux paramètres
supplémentaires que la méthode va avoir par lambda-lifting dus aux
méthodes dont elle dépend en provenance de l'espèce elle-même ou des
paramètres de cette espèce.

Elle extrait les méthodes de l'espèce courante dont le champ
decl-dépend, les méthodes des paramètres de l'espèce dont le champ
decl-dépend et finalement elle synthétise la liste des paramètres
formels que la méthode va avoir du fait de ces lambda-liftings dus aux
dépendances.

Cette dernière liste va être directement imprimée par le générateur de
code mais va également être retournée afin d'être enregistrée dans le
contexte de compilation. Ainsi, l'application récursive d'une méthode
devra et sera capable d'utiliser ces paramètres supplémentaires en
plus de ceux effectivement passés lors de l'appel dans le code FoCaL.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Algo}
\begin{enumerate}
  \item On récupère dans le graphe de dépendances de l'espèce (qui se
    trouve dans le contexte de compilation), les méthodes dont on
    decl-dépend directement (pas transitivement).
  \item On récupère la liste des méthodes en provenance des paramètres
    de l'espèce et dont on dépend. Cette liste est une association
    (nom de paramètre d'espèce, nom de méthode de ce paramètre dont on
    dépend).
  \item On construit la chaîne de caractères (le ``code OCaml'') à
    émettre pour énumérer les noms de paramètres lambda-liftés
    (concaténation des noms séparés par un espace). \\
    On commence par ceux induits par les méthodes de paramètres de
    l'espèce. On les nomme ``\_p\_'' + le nom du paramètre d'espèce
    sans majuscule + le nom de la méthode. \\
    Ensuite, on continue avec les paramètres induits par les
    méthodes de l'espèce courante dont on dépend. On les nomme
    ``abst\_'' + le nom de la méthode. \\
  \item On retourne les 3 informations calculées ci-dessus, elles
    seront utilisées ailleurs dans la suite de la génération du code.
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{generate\_one\_field\_binding}
Cette fonction émet réellement le code OCaml correspondant au champ de
l'espèce. Le champ n'est généré que s'il est défini dans l'espèce
courante, à son niveau d'héritage. S'il est hérité alors on ne
re-génère pas son code.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Algo}
\begin{enumerate}
  \item Si le champ est hérité on ne fait rien, sinon\ldots
  \item En fonction de si on provient un champ {\tt let}, du premier
    binding d'un champ {\tt let rec} ou d'un suivant d'un champ
    {\tt let rec}, on génère le lieur OCaml {\tt let}, {\tt let rec}
    ou {\tt and}.
  \item On imprime la chaîne de caractères représentant les paramètres
    supplémentaires dus aux lambda-liftings (chaîne construite par la
    fonction {\tt compute\_lambda\_liftings\_for\_field} vue
    précédemment.
  \item On imprime chaque paramètre de l'identificateur lié en
    l'annotant par son type.
  \item On imprime le ``='' puis on génère la traduction du corps (en
    invoquant \\ {\tt Base\_exprs\_ml\_generation.generate\_expr}).
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{generate\_collection\_generator}
Cette fonction est appelée uniquement dans le cas où l'espèce compilée
est complètement définie. Elle va créer la fonction générateur de
collection dont le travail sera de construire une valeur du type
record représentant l'espèce, pour peu que l'on lui fournisse les
fonctions instanciant les méthodes dont les méthode du record
dépendent. Le générateur de collection d'une espèce est toujours
appelé {\tt collection\_create}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Algo}
\begin{enumerate}
  \item On commence par générer le nom du générateur, en lui
    adjoignant toujours un argument ``artificiel'' {\tt ()}. Ceci est
    dû au fait que si ce générateur se trouve ne pas avoir de
    paramètre du tout (par exemple, l'espèce n'a pas de paramètre donc
    ne peut pas avoir de méthodes dont on dépend en leur provenance,
    de plus étant totalement définie elle n'a plas non plus de 
    donc étant complètement définie elle n'a pas non plus de paramètre
    représentant des méthodes d'elle-même non encore définie), alors
    il va se retrouver comme étant directement égal à l'expression de
    record représentant l'espèce en OCaml. \\
    Or pour des raisons de ``non-expansivité'', il est in terdit en ML
    de généraliser des valeurs non fonctionnelles (en simplifiant un
    peu). De ce fait, les éventuelles variables de type présentes dans
    le type du générateur de collection ne pourront pas être
    généralisées, ce qui pourra empêcher l'utilisation de ce dernier
    de manière polymorphe. \\
    En rajoutant toutle temps ce paramètre {\tt ()}, on est sûr que le
    générateur de collection est bien une fonction et donc sera
    généralisable. L'exemple ``tests.ok\_list.foc'' illustre ce
    problème. On pourrait ne rajouter ce paramètre que lorsque l'on
    s'aperçoit que le générateur de collection n'a pas de paramètre
    mais c'est plus simple de le mettre tout le temps. Si un jour on
    veut ``optimiser'', on verra\ldots
  \item On génère les paramètres dont le générateur de collection
    aura besoin pour construire chacune des fonctions locales
    (fonctions correspondant chaque méthode effective stockée dans le
    record de la collection) via la fonction
    ({\tt dump\_collection\_generator\_arguments}).
  \item On récupère la liste liant les paramètres d'espèces liés aux
    noms de méthodes qu'ils apportent (et donc qui devront être
    instanciées) et dont on a besoin pour appliquer le générateur de
    collection.
  \item Pour chaque methode de l'espèce, on génère une fonction locale
    correspondant l'application du générateur de méthode. C'est cette
    fonction locale qui va être stocké dans le record. La fonction
    locale correspondant à une méthode est nommée ``local\_'' + le nom
    de cette méthode. C'est ce que fait la fonction
    {\tt process\_one\_field} locale à
    {\tt generate\_collection\_generator}.
  \item Finalement, on affecte à chaque champ du record (dont le nom
    correspond au nom d'une méthode de l'espèce), la fonction locale
    nommée ``local\_'' + le nom de cette méthode.
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{process\_one\_field}
\begin{enumerate}
  \item On commence par rechercher le générateur de méthode à
    appliquer. Si la méthode est définie au niveau courant de
    l'héritage, alors il est une fonction localement définie au
    module, du même nom que la méthode. Si la méthode provient de
    l'héritage, alors on regarde de quelle espèce et le nom du
    générateur est le nom de la méthode mais qualifié par le module
    représentant l'espèce d'où il provient. Un fois qu'on a son nom,
    on peut l'imprimer.
    \item On applique maintenant le générateur de methode trouvé à
      chacun de ses paramètres supplémentaires induits par les divers
      lambda-liftings que nosu avons précédemment réalisés. \\
      On commence par les paramètres dus aux méthodes des paramètres
      de l'espèce. Le schéma de nommage que l'on utilise est le même
      que celui utilisé lorsque nous avons lambda-lifté: ``\_p\_'' +
      le nom du paramètre d'espèce + ``\_'' + le nom de la
      méthode. L'ordre d'application de ces paramètres est gardé
      cohérent avec celui de la définition du générateur de méthode
      car nosu avons pris soin, lors de la création de ces
      générateurs, de conserver l'information de cet ordre (champ
      {\tt cfm\_dependencies\_from\_parameters} associé à chaque champ
      à compiler de l'espèce.
    \item Ensuite, on applique les paramètres correspondant aux
      méthodes de l'espèce elle-même. Ces méthodes ont donné lieu à
      des fonctions locales nommées ``local\_'' + le nom de la
      méthode. Comme on a également conservé l'ordre de ces arguments
      lors de la création du générateur de méthode, on a juste à
      parcourir la liste ordonnée de ces méthodes dont on dépend et de
      générer l'argument nommé ``local\_'' + le nom de la méthode.
\end{enumerate}
