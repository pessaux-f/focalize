% $Id: phd_changes.tex,v 1.4 2009-03-16 14:22:57 pessaux Exp $
...
and [PRM], it should be stated that implicitly the parameter $C_{p'}$
...
% Donc voilà l'état de la bavure. Maintenant, quelques remarques sur des questions que l'on s'était posées avec toi, David...

% 1) C'est donc le Section externe, celle de l'espèce qui gêne. En effet, lorsque l'on sort de la Section interne, le théorème est bien abstrait par rapport à la Variable "interne".

% 2) Ca ne se produit que sur les Theorem car ce sont les seuls à être dans une Section imbriquée.

% 3) De la nécessité de toujours générer les Local self_xxx (on en avait parlé hier David): comme on ne lambda-lifte pas dans les property (Hypothesis Coq), si une property dépend d'une méthode, il faut bien qu'elle ait de quoi en parler dans l'énoncé de la property. Et c'est justement par ce self_xxx.

% 4) Maintenant, la question est de savoir si l'on pourrait s'affranchir de toutes ces me.des si on gérait les abstractions à la pogne, c'est-à-dire en lambda-liftant nous-mêmes, en virant les Sections, et en généralisant ces lambda-liftings aux property et theorems.
% Par contre, je ne sais pas jusqu'où ça va nous emmener. En particulier, pour Zenon, qu'est-ce qu'il va lui arriver une fois que toutes les property et theorem seront passés à l'ordre sup...

% LocalWords:  th eq lookatme
