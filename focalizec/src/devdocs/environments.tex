% $Id: environments.tex,v 1.8 2009-03-27 13:40:10 pessaux Exp $
...
program, i.e. to infer types where they are not writen, to check that
explicitly writen types are correct and to annotate the AST with the
...
type is the one hosting the constructor.
...
application and type-check this application like any other one.

So, for each constructor of a sum type, we record its type scheme
({\tt cstr_scheme}) and if it has arguments or not (.
\subsubsection{Record type field names}
