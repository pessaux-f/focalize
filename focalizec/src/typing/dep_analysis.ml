	  ...
	  | Parsetree.EI_method (coll_specifier_opt, vname) ->
	      ...
	      match coll_specifier_opt with
		   ...
		   match coll_specifier with
			...
			if current_species = (module_name, coll_name) then
			  (* Case c!x in Virgile Prevosto's Phd, section 3.5, *)
			  (* page 30, definition 12.                          *)
			  VnameSet.singleton vname
			else VnameSet.empty
	...
	| Some coll_specifier ->
	   ...
	   match coll_specifier with
		...
		(* match the [current_species] then are are still in the     *)
		  ...
		  VnameSet.empty (* Method from another species, then no dep. *)
		end)
	   end)
       end)
;;
...
let field_only_decl_dependencies ~current_species = function
  ...
  | Env.TypeInformation.SF_let_rec l ->
      ...
      let names_of_l =
	List.fold_left
	  (fun accu_set (_, n, _, _, _) -> VnameSet.add n accu_set)
	  VnameSet.empty
	  l in
  ...
  List.fold_right
    (fun field accu ->
	  ...
	  if vname = field_name then Handy.list_cons_uniq_eq vname accu
	   ...
	   if List.exists (fun (_, vname, _, _, _) -> vname = field_name) l then
	     List.fold_right
	       (fun (_, n, _, _, _) accu' -> Handy.list_cons_uniq_eq n accu')
	       l accu
	   else accu)
    fields
    []
...
let names_set_of_field = function
  ...
  | Env.TypeInformation.SF_let_rec l ->
      List.fold_left
	(fun accu_names (_, n, _, _, _) -> VnameSet.add n accu_names)
	VnameSet.empty
	l
;;
...
let find_most_recent_rec_field_binding y_name fields =
  ...
  let rec rec_search = function
	...
	match h with
	 ...
	 | Env.TypeInformation.SF_let_rec l ->
	     if List.exists (fun (_, n, _, _, _) -> n = y_name) l then h
	     else rec_search q
	end) in
...
let in_species_decl_dependencies_for_one_function_name ~current_species
  ...
  if List.for_all
      (function
	| Env.TypeInformation.SF_sig (_, _, _)
	| Env.TypeInformation.SF_let (_, _, _, _, _) -> true
