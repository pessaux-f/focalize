(* $Id: graph.ml,v 1.1 2008-09-22 11:55:33 habib Exp $ *)
  ...
  match list with
   ...
   | hd :: tl ->
       if hd.n_object = o && hd.n_access = a then
         if equal_star hd s d then true
         else
           if isInList2 hd.n_children s d then true
           else isInList tl s o a d
       else isInList tl s o a d
;;
...
let rec nouveau_noeud graph s o a d =
  match graph with
   | [] ->
       let r = { b_subject = s; b_depth = d; b_children = [] } in r
   | hd :: tl ->
	 ...
	 match hd.n_children with
	  ...
	  | hd2 :: tl2 ->
	      (begin
	      if hd2.b_subject = s && hd2.b_depth = d then hd2
	      else
...
let rec create_f_a graph =
        match graph with
                [] -> []
                | hd::tl -> let list_a = (hd.n_subject, hd.n_object,
                                        hd.n_access, hd.n_depth)
                                 in list_a::create_f_a tl
;;
*)
(*Version compilable*)
(*
let rec create_f_d2 s o a list =
        match list with
                [] -> []
                | (s_aux,d)::tl -> let list_aux = (s, o, a, s_aux, d)
                                 in list_aux::create_f_d2 s o a tl
;;
*)
...
let rec create_f_d2 s o a list_read list_write =
        match list_read with
                [] -> []
                | (s_aux,d)::tl -> list_write <- (s, o, a, s_aux, d)::create_f_d2 s o a tl list_write
;;
*)

listD1 ;;
trier listD1 ;;
let list_a = delegate_a listA in delegate_d list_a listD1 ;;
create_g listA listD1 ;;
let list_a = create_g listA listD1 in isInList list_a "Marie" "f1" "x" 3 ;;
