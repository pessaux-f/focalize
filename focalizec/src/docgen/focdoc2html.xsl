...
<xsl:template match="foc:definition|foc:signature|foc:theorem|foc:property|foc:carrier|foc:letprop|foc:global-fun|foc:concrete-type">
  ...
  <xsl:if test="(($verbose = 0) and ($initial-apparition=$nameSpecies)) or $verbose >= 1  or name(..)='foc:collection'
		or name(..)='foc:focdoc'">
    <tr>
...
<!--
     ...
     <ins> 
      <a>
        <xsl:attribute name="href">
            <xsl:value-of select="$docpath"/>
            <xsl:if test="foc:comes-from[@infile]">
              <xsl:value-of select="normalize-space(foc:comes-from/@infile)"/>
		<xsl:value-of select="$ext"/>
	    </xsl:if>
	          <xsl:text>#species-</xsl:text>
        	  <xsl:value-of select="normalize-space(foc:comes-from)"/>
        </xsl:attribute>
        <xsl:value-of select="normalize-space(foc:comes-from)"/>
      </a>
      </ins>
	...
	<a>
        <xsl:attribute name="href">
        <xsl:value-of select="$docpath"/>
	<xsl:if test="foc:initial-apparition[@infile]">
		<xsl:value-of select="normalize-space(foc:initial-apparition/@infile)"/>
		<xsl:value-of select="$ext"/>	
	</xsl:if>
		<xsl:text>#species-</xsl:text>
	        <xsl:value-of select="normalize-space(foc:initial-apparition)"/>
	 </xsl:attribute>
	      <xsl:value-of select="normalize-space(foc:initial-apparition)"/>
	</a>
	</ins>
